...
The traffic dataset contains continuous hourly measurements. Similar to the population view, the latest timestamp per year was identified (e.g., November 2025), and all traffic counts from that timestamp were aggregated to city level.

```sql
CREATE VIEW v_traffic_city_yearly AS
SELECT
    YEAR(t.timestamp) AS year,
    SUM(t.vehicle_count) AS vehicles_city
FROM trafficmeasurement t
JOIN (
    SELECT 
    ...
    ON YEAR(t.timestamp) = latest.year
   AND t.timestamp = latest.latest_timestamp
GROUP BY YEAR(t.timestamp)
ORDER BY year;
```

### Combined View for Comparative Analysis

For visualization in tools such as Metabase, a combined view was created that aligns the yearly population totals with the corresponding yearly traffic totals. Additionally, the view computes the year-over-year traffic growth to enable comparative trend analysis between demographic development and traffic intensity.

```sql
CREATE VIEW v_city_population_traffic AS
SELECT
    p.year,
    p.population_city,
    t.vehicles_city,
    ROUND(
        (t.vehicles_city - LAG(t.vehicles_city) OVER (ORDER BY p.year))
        / LAG(t.vehicles_city) OVER (ORDER BY p.year) * 100,
        2
