...
# The Datasets
Please allow us using our presentation slide to better introduce our datasets.

```{r data_landscape, echo=FALSE, out.width="90%", fig.cap="Data Landscape Overview", fig.pos="H", fig.align="center"}
knitr::include_graphics("materials_for_report_later/data_landscape_insert_report.png")
```
...
Because the raw dataset included a mixture of analytical attributes and highly technical metadata, each field was examined to determine its relevance for traffic-flow analysis. The review was guided by the perspective of the City of Zurich’s urban planning department
### Removed Fields
...
```{r table_screenshot, echo=FALSE, out.width="40%", fig.cap="Relational database schema of the original data model in MySQL Workbench, illustrating the core entities."}
...
```{r views_screenshot, echo=FALSE, out.width="45%", fig.cap="Overview of analytical SQL views in MySQL Workbench, including both KPI-specific and supporting views"}
...
The final layer of the project is the delivery of actionable insights through an interactive Metabase dashboard. By connecting Metabase to the optimized MySQL views and materialized tables, we allow urban planners to explore the data dynamically without writing SQL code.

## Metabase Dashboard Concept

The dashboard is designed to visualise the core questions & insights defined in our KPI framework. It is divided into three visual topics:

### 1. Citywide Overview (KPI 0)
...
**Key Insight:** While the population has grown steadily by over 14%, traffic intensity dropped significantly in 2020 and has remained below pre-pandemic levels, suggesting a shift in mobility behavior.
### 2. District Pressure Map (KPI 1)
- **Visual:** A table grouping districts by their "Stress Index" classification (Commuter Pressure vs. Residential Pressure vs Insufficient data).
- **Insight:** Identifies specific districts that may face high commuter inflows (Traffic Growth > Population Growth) and let our client know, which area they should focus on implementing urban design and infraestructure measures.
- **Decision Recommendation for Client:** Targeted infrastructure investment improving roads in "Commuter" zones. Focus on living comfort design in "Residential" growth zones.
[please insert the screenshot of the dashboard here, and adjust the description]

### 3. Operational Bottlenecks (KPI 2 & 3)
...
- **Decision Support:** The bottlenecks are the critical areas that the client should focus as painpoints for priority. And a directional imbalance is somewhere, the client could implement hourly traffic control measures(change lane numbers on one direction depending on time of the day).
[please insert the screenshot of the dashboard here, and adjust descriptions]


# Conclusions & Lessons Learned

## Project Achievements
This project successfully transformed raw, converted open data into a robust analytical insights for the City of Zurich.
1.  **Data Design & Integration:** We consolidated over 21 million traffic records with demographic and spatial data into a clean 3NF schema.
2.  **Analysis Concept Design:** The "Stress Index" provides a practical, comparable metric for urban analysis and provides practical insights for stakeholders.
3.  **Performance Optimization:** We learned that "Big Data" in a relational database requires careful designing. By implementing indexing and SARGable queries that we learned in course, we reduced query times from minutes to seconds. This enabled us to build a dashboard that provides real-time reporting.
...
However, this success is not geographically uniform. Our **Stress Index** highlights a divide:
*   **Commuter Funnels:** Specific quarters like **Enge (District 2, +150.2% Stress)** and **Fluntern (District 7, +92.0% Stress)** are acting as pressurized entry valves, their traffic growth vastly outpaces population. These areas require intervention or P+R incentives.
*   **Residential Success Zones:** Conversely, **Escher Wyss (District 5, -134% Stress)** and **Altstetten (District 9, -73.3% Stress)** might be and example of successful urban living. In Escher Wyss, population nearly doubled (+89%) while traffic dropped (-45%), proving that high-density living can be achieved with low car dependency. However, local context need to be explored, other reasons might be present.
...
## Lessons Learned
- **Data Structure Design is Important:** A significant portion of time was spent handling the design of the database structure. Another data scientist usual topic is also the time consuming task of data quality & data cleaning
- **Optimization should be done Upfront:** Initial attempts to run aggregations on the full 20-million-row dataset were frustratingly slow. Data query efficiency, is better implemented upfront.
- **Value of Staging:** using a two-stage ETL process (Staging -> Production) saved us from multiple "start over" moments.
- **Teamwork:** We used github to store scripts and miro to share ideas and progress. We would be still happy to hear other inspiration of teamwork with database management. We think that the current setup for teamwork though necessary, but could be improved.
- **Gen-AI for Database:** Since mysql and metabase are not integrated with AI, we relied on web-based AI tools, we find that, it is helpful but not optimal due to context limitation. We intend to find better AI usage for better context engineering while working with databases.

# Individual Team Member Reflections

### Ramiro Díez-Liébana
- [to be added]
### Valeska Blank**Key Learning:** 
[to be added]
### Dongyuan Gao
- This project is my first database project and experience. I learned through hands on experience how a data structure is designed and how a database setup with mysql(via vm) works, and how we can connect it to a frontend tool(metabase).
- A very important lesson i learned is we should always consider the efficiency in mind, while writing SQL queries, and designing the database. Otherwise, it wouldnt be possibole to have meaningful usecase, especially for the frontend visualisations.
- One aspect that I would still need to discover in future project is, how to collab with team when we do a database project like this. Since queries and database desgins are not easy to communicate with team, we used git for collaboration, to keep the scripts and concepts in files there. It is still not easy to find bugs or improve it based on teammates work, comparing to code review in programing which I am used to.Therefore, I am still not sure how it is done in practice. Or are queries more for one time usage for teammembers? This is the question that I will keep in mind for future projects.
- Another note about teamwork, in my experience, it would be better to already have a metabase or other data visual tool in place, while working on the project, instead of implementing it in the end. Then it will be easier to have the data results ready for inspect, during the work.
