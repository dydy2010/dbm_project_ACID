...
We intend to analyze the population & traffic data over the years of Zurich, and try to find where the urban planning could be improved, including traffic usecases and urban living usecases.
...
All **SQL scripts**, **ETL code**, and **documentation** are **version-controlled** and available at: https://github.com/dydy2010/dbm_project_ACID

### Metabase Connection Configuration

Metabase was connected to the MySQL database using the settings shown below (**password hidden**). In our setup, Metabase connects to **host** `localhost` on **port** `3306`, using **database name** `traffic_population_zh` and **username** `root`.

```{r metabase_configuration, echo=FALSE, fig.cap="Metabase database connection configuration (password hidden)", fig.align='center', out.width='45%', fig.pos='H'}
knitr::include_graphics("materials_for_report_later/screenshots/metabase_configuration.png")
```
...
### Physical-Conceptual Schema Matching

The following table shows how each conceptual entity maps to its physical implementation in MySQL:

| Conceptual Entity | Physical Table | Key Differences / Implementation Details |
|-------------------|----------------|------------------------------------------|
| CountingSite | `CountingSite` | `axis` column dropped due to consistent inconsistency |
| MeasurementSite | `MeasurementSite` | Normalized 1:n relation to CountingSite |
...
| Population | `Population` | No surrogate key; uses composite natural key |
| Sex | `Sex` | Lookup table extracted from population attributes |
| Origin | `Origin` | Lookup table extracted from population attributes |
| TrafficSignal | `TrafficSignal` | Lookup table extracted from site metadata |
...
*   **Before Optimization**: Execution times ranged from **2 to 10 minutes** per view. Non-SARGable predicates caused full table scans on 21M+ rows, making direct query use in Metabase unsuitable for interactive analysis.
*   **After Optimization**: View execution times reduced to **seconds**. Targeted indexes and SARGable rewrites enabled efficient index-based access paths. Materialized tables were added as a final layer to allow instant dashboard filtering.

    ```{r index_runtime_screenshot_kpi1, echo=FALSE, fig.pos="H", out.width="100%", fig.cap="After Optimization: Execution log showing view creation times reduced to seconds (e.g., v\\_kpi1\\_quarter\\_stress\\_index\\_new)."}
...
1. **Created targeted indexes**:
    ...
    ```
    These two lines (**run once**) already **drastically reduce** the **query time** for the views, like in above screenshot shows.
...
4. **Materialized Aggregation Tables**
...
The following query represents the **core aggregation logic** used for **KPI 2** and was used to generate the **Visual EXPLAIN** shown below. **Note that this query is shown in its optimized (SARGable) form; the original view used `YEAR(tm.timestamp)`.**
...
knitr::include_graphics("materials_for_report_later/screenshots/dashboard_with_filter.png")
```

The Metabase Dashboard is designed to visualize the core questions & insights defined in our KPI framework. With the **quarter filter** user can **interactively** see the related quarter data and visuals.
It is divided into **three visual topics**:
...
**Key Insight:** While the population has grown steadily by over 14%, traffic intensity dropped significantly in 2020 and has remained below pre-pandemic levels, suggesting a shift in mobility behavior. This is backgroud information and overview for decision makers.

## Quarter Pressure Map (KPI 1)
This table groups quarters by their "Stress Index" classification (Commuter Pressure, Residential Pressure, Insufficient data and Balanced).
...
- **Key Insight:** Identifies specific quarters that may face **high commuter inflows** (**Traffic Growth > Population Growth**) and let our client know, which area they should focus on implementing **urban design** and **infrastructure measures**.

- **Decision Recommendation for Client:**  In the filtered out quarters, investigate into the cases with **local context**. If necessary, invest toward **_commuter-pressure_** zones requiring **road capacity expansion** and **traffic management**. In **_residential-pressure_** quarters, improve **public transit** and other mobility options such as improved **cycle-friendly roads**.


## KPI 2: Peak-Hour Bottlenecks

- **Visual:** A list of **"Red Flag" intersections** where the **peak-hour volume** consistently exceeds **700 vehicles/hour**.
...
```
- **Key Insight**: **Ten intersections** consistently exceed **operational capacity** during **peak hours**, with **afternoon rush (16:00-17:00)** representing the **primary constraint period**. These bottlenecks create **systematic delays** and potentially degrade **network performance** across surrounding areas, if the volume is not justified.

- **Decision Recommendation for Client**: Investigate in to these areas, if the volume of peak-hour is not justified, some bottlenecks could be **highway entrance**, so that not all of them are really problematic.
For unjustified volume cases, apply **short-term interventions** (**signal optimization**, **traffic management**) at **flagged intersections** immediately. To reduce **peak concentration**, consider expanding capacity of **bus lanes** or an **adaptive traffic signal system**.

## KPI 3: Directional Imbalances

- **Visual:** A breakdown of directional imbalance for major intersections.
...
- **Key Insight:** Several intersections exhibit strong directional dominance (>60% flow in one direction).

- **Decision Support:** The bottlenecks are the **critical areas** that the client should focus as painpoints for priority. Unless they are one direction lane or there are flow management already in place, the data indicate potential for **tidal flow management** to **optimize capacity** during **peak periods**(**if measures not yet in place**). 
...
Instead of reimplementing the database in MongoDB, we made use of our finalized SQL data model to **Extract** the verification datasets and imported into MongoDB.
...
knitr::include_graphics(c("materials_for_report_later/FinalScreenShots/ZurichAverageVolumeDistrictMap.jpeg", "materials_for_report_later/FinalScreenShots/AverageVolumeNoSQL.jpeg"))
```
...
For example, **Brunaustrasse (85% dominance toward Enge)** and **Hottingerstrasse (71% dominance toward Heimplatz)** are prime candidates for **dynamic tidal flow systems**. Implementing reversible lanes here could unlock peak-hour capacity without the cost and disruption of infrastructure constructions.
...
- **Value of Staging Layers:**  
  Using a **two-stage ETL process** (Staging to Production) proved highly valuable. This separation allowed us to iteratively **clean and transform data** without repeatedly **rebuilding the entire schema**, preventing several potential **“start-over” situations**.
...
One limitation of our analysis is that some records could not be fully assigned to a clear geographic unit (quarter or district) due to **missing** or **"unknown" values** in the source data. This reduces **coverage** for some areas and can slightly **bias** KPI comparisons.

In a follow-up project, we could improve **completeness** by enriching the dataset with additional **open data sources** and by applying **statistical** or **machine-learning based imputation methods**. However, since this is a **focused database project**, we decided not to implement **advanced imputation**.

# Individual Team Member Reflections

 **Ramiro Díez-Liébana**

- **Real-World Data**: I learned that **messy**, especially **sensor data** demands flexible solutions beyond standard cleaning steps. **Manual street name mappings** and **complex addresses** taught me that **automation has limits** when dealing with **inconsistent real-world datasets**.

- **Metabase  Data  Insights**: I liked that Metabase's features, although open-source, work great translating our SQL queries into **intuitive visualization tools** for **urban planners**. You can create deep visuals, without need of prior experience. **Loading power** however was a bit limited, with bigger projects taking longer and being more cumbersome to work with.
...
- **Challenges of Team Collaboration in Database Projects:** One aspect that I would still need to discover in future project is how to **collaborate with team** when we do a database project like this. Since queries and database designs are not easy to communicate with team, we used **git for collaboration**, to keep the scripts and concepts in files there. It is still not easy to find bugs or improve it based on teammates work, comparing to **code review in programming** which I am used to. Therefore, I am still not sure how it is done in practice. Or are queries more for one time usage than for team members? This is the question that I will keep in mind for future projects.
...
- City of Zurich Open Data: *Bevölkerung nach Monat, Stadtquartier, Geschlecht, Altersgruppe und Herkunft* (population per month by city quarter, sex, age group, and origin). <https://miro.com/app/board/uXjVJuBJnN8=/?moveToWidget=3458764643324310209&cot=14>

- City of Zurich Open Data: *Daten der Verkehrszählung zum motorisierten Individualverkehr (Stundenwerte), seit 2012* (motorised traffic counts, hourly values, since 2012). <https://data.stadt-zuerich.ch/dataset/sid_dav_verkehrszaehlung_miv_od2031/resource/c1855626-88e1-4d48-99c8-00b049aae900>

- City of Zurich Open Data: *Geo-Adressen Stadt Zürich* (address reference dataset used for spatial linking). <https://data.stadt-zuerich.ch/dataset/geo_adressen_stadt_zuerich>

# Generative AI Declaration & Guidelines {-}

Generative AI was used as a supplementary tool on top of the supporting material provided in the DBM lecture.
