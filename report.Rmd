...
The database system was deployed on a university-hosted Windows virtual machine and is accessible via the internal university network.

**Virtual Machine Access:**

Host: win-hs-007.dbm.ls.eee.intern
Username: labadmin  
Password: provided by the instructors via ILIAS

**Database (MySQL):**

Database: traffic_population_zh  
Access is configured using project-specific database users.

**Metabase:**

Metabase is installed and executed locally on the virtual machine. After logging into the VM, the application can be started and accessed via:
URL: http://localhost:3000

For security reasons, project-specific credentials for MySQL and Metabase are not included in this report. Evaluation credentials can be provided to the instructor upon request.
...
3. **Explanation helper** – `sql scripts_optimized/run_explain_checks.sql` contains `EXPLAIN` statements for each KPI view. After running the indexes and views we expect `type = range` or `ref`, never `ALL`.
